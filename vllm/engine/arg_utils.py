...
class EngineArgs:
    ...
    hyquant_top_ratio: float = CacheConfig.hyquant_top_ratio
    hyquant_group_size: int = CacheConfig.hyquant_group_size
    hyquant_window_size: int = CacheConfig.hyquant_window_size
    hyquant_retire_interval: int = CacheConfig.hyquant_retire_interval
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        cache_group.add_argument(
            "--hyquant-top-ratio", **cache_kwargs["hyquant_top_ratio"]
        )
        cache_group.add_argument(
            "--hyquant-group-size", **cache_kwargs["hyquant_group_size"]
        )
        cache_group.add_argument(
            "--hyquant-window-size", **cache_kwargs["hyquant_window_size"]
        )
        cache_group.add_argument(
            "--hyquant-retire-interval",
            **cache_kwargs["hyquant_retire_interval"],
        )
        cache_group.add_argument(
    ...
    def create_engine_config(
        ...
        cache_config = CacheConfig(
            ...
            hyquant_top_ratio=self.hyquant_top_ratio,
            hyquant_group_size=self.hyquant_group_size,
            hyquant_window_size=self.hyquant_window_size,
            hyquant_retire_interval=self.hyquant_retire_interval,
        ...
        if resolved_cache_dtype == "hyquant_k4v4":
            block_size = cache_config.block_size
            if self.hyquant_group_size % 2:
                raise ValueError("--hyquant-group-size must be even")
            if self.hyquant_window_size % block_size:
                raise ValueError(
                    "--hyquant-window-size must be divisible by the KV cache "
                    f"block size ({block_size})."
                )
            if self.hyquant_retire_interval % block_size:
                raise ValueError(
                    "--hyquant-retire-interval must be divisible by the KV "
                    f"cache block size ({block_size})."
                )
            if self.enable_prefix_caching:
                if self.prefix_match_unit is None:
                    self.prefix_match_unit = block_size
                    cache_config.prefix_match_unit = block_size
                elif self.prefix_match_unit != block_size:
                    raise ValueError(
                        "HyQuant prefix caching supports full physical-block "
                        "hits only; --prefix-match-unit must equal the KV cache "
                        f"block size ({block_size})."
                    )
            ...
            if self.kv_cache_dtype_skip_layers:
                unsupported_features.append("per-layer KV dtype skipping")
            if self.kv_offloading_size is not None:
                unsupported_features.append("KV offloading")
            if self.kv_transfer_config is not None:
                unsupported_features.append("KV connector/transfer")
            if self.decode_context_parallel_size > 1:
                unsupported_features.append("decode context parallelism")
            if unsupported_features:
                raise ValueError(
                    "HyQuant does not support "
                    + ", ".join(unsupported_features)
                    + ". Disable these features before selecting "
