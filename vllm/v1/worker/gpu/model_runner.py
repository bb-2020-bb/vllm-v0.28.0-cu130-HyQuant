...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def prepare_inputs(
        ...
        hyquant_state_slots = None
        hyquant_state_slots_cpu = None
        hyquant_prompt_lens = None
        hyquant_prompt_lens_cpu = None
        hyquant_cached_prefix_lens = None
        hyquant_cached_prefix_lens_cpu = None
        if self.cache_config.cache_dtype == "hyquant_k4v4":
            state_slot_buffer = self.input_buffers.hyquant_state_slots
            prompt_len_buffer = self.input_buffers.hyquant_prompt_lens
            cached_prefix_buffer = self.input_buffers.hyquant_cached_prefix_lens
            state_slot_cpu_buffer = self.input_buffers.hyquant_state_slots_cpu
            prompt_len_cpu_buffer = self.input_buffers.hyquant_prompt_lens_cpu
            cached_prefix_cpu_buffer = (
                self.input_buffers.hyquant_cached_prefix_lens_cpu
            )
            state_slot_buffer[:num_reqs].copy_(idx_mapping)
            state_slot_buffer[num_reqs:num_reqs_padded].fill_(-1)
            state_slot_cpu_buffer[:num_reqs].copy_(
                torch.from_numpy(idx_mapping_np)
            )
            state_slot_cpu_buffer[num_reqs:num_reqs_padded].fill_(-1)
            torch.index_select(
                self.req_states.prefill_len.gpu,
                0,
                idx_mapping,
                out=prompt_len_buffer[:num_reqs],
            )
            prompt_len_buffer[num_reqs:num_reqs_padded].zero_()
            prompt_len_cpu_buffer[:num_reqs].copy_(
                torch.from_numpy(batch_req_state.prefill_len_np)
            )
            prompt_len_cpu_buffer[num_reqs:num_reqs_padded].zero_()
            torch.index_select(
                self.req_states.hyquant_cached_prefix_len.gpu,
                0,
                idx_mapping,
                out=cached_prefix_buffer[:num_reqs],
            )
            cached_prefix_buffer[num_reqs:num_reqs_padded].zero_()
            cached_prefix_len_np = self.req_states.hyquant_cached_prefix_len.np[
                idx_mapping_np
            ]
            cached_prefix_cpu_buffer[:num_reqs].copy_(
                torch.from_numpy(cached_prefix_len_np)
            )
            cached_prefix_cpu_buffer[num_reqs:num_reqs_padded].zero_()
            hyquant_state_slots = state_slot_buffer[:num_reqs_padded]
            hyquant_state_slots_cpu = state_slot_cpu_buffer[:num_reqs_padded]
            hyquant_prompt_lens = prompt_len_buffer[:num_reqs_padded]
            hyquant_prompt_lens_cpu = prompt_len_cpu_buffer[:num_reqs_padded]
            hyquant_cached_prefix_lens = cached_prefix_buffer[:num_reqs_padded]
            hyquant_cached_prefix_lens_cpu = cached_prefix_cpu_buffer[
                :num_reqs_padded
            ]
        ...
        input_batch = InputBatch(
            ...
            hyquant_state_slots=hyquant_state_slots,
            hyquant_prompt_lens=hyquant_prompt_lens,
            hyquant_cached_prefix_lens=hyquant_cached_prefix_lens,
            hyquant_state_slots_cpu=hyquant_state_slots_cpu,
            hyquant_prompt_lens_cpu=hyquant_prompt_lens_cpu,
            hyquant_cached_prefix_lens_cpu=hyquant_cached_prefix_lens_cpu,
            max_query_len=(
    ...
    def _hyquant_retirement_boundary_needed(
        self, batch_req_state: "BatchReqState | None"
    ) -> bool:
        """Return whether this step publishes a new compact HyQuant page.

        HyQuant page retirement is intentionally host-side: the retiring block
        is selected from the BF16 hot ring and packed into the compact cache
        before the attention kernel reads it.  A FULL CUDA-Graph replay cannot
        execute that Python operation, so the single boundary step must use
        eager execution.  Keeping the check here makes ordinary decode steps
        graph-safe without adding a per-layer host synchronization.
        """
        if (
            batch_req_state is None
            or self.cache_config.cache_dtype != "hyquant_k4v4"
            or not batch_req_state.num_tokens
        ):
            return False

        # The default optimized policy launches GPU retirement only when this
        # step actually crosses a retirement boundary.  A compatibility switch
        # keeps the former every-step graph-safe no-op launch available for
        # controlled A/B measurements.
        if (
            os.environ.get("VLLM_HYQUANT_GPU_RETIRE", "1") != "0"
            and os.environ.get("VLLM_HYQUANT_GPU_RETIRE_EVERY_STEP", "0")
            == "1"
        ):
            return False

        block_size = int(self.cache_config.block_size)
        window_size = int(self.cache_config.hyquant_window_size)
        retire_interval = int(self.cache_config.hyquant_retire_interval)
        if block_size <= 0 or window_size <= 0 or retire_interval <= 0:
            return False

        scheduled = batch_req_state.num_scheduled_tokens
        prefill_lengths = batch_req_state.prefill_len_np
        prefilling = batch_req_state.is_prefilling_np
        computed_tokens = self.req_states.num_computed_tokens_np

        for batch_index, token_count in enumerate(scheduled):
            # Prefill pages are packed as part of the prefill forward itself;
            # this guard is only for requests already past their prompt.
            if int(token_count) <= 0 or bool(prefilling[batch_index]):
                continue
            request_index = int(batch_req_state.idx_mapping_np[batch_index])
            prompt_length = int(prefill_lengths[batch_index])
            previous_length = int(computed_tokens[request_index])
            current_length = previous_length + int(token_count)

            initial = max(prompt_length - window_size, 0) // block_size * block_size

            def committed(
                length: int,
                request_prompt_length: int = prompt_length,
                request_initial: int = initial,
            ) -> int:
                generated = max(length - request_prompt_length, 0)
                retired = generated // retire_interval * retire_interval
                max_allowed = (
                    max(length - window_size, 0) // block_size * block_size
                )
                return min(request_initial + retired, max_allowed)

            if committed(current_length) > committed(previous_length):
                return True
        return False
    ...
    def execute_model(
        ...
        # FULL graph replay cannot run HyQuant's host-side page retirement.
        # Force only the boundary step eager; all other one-token decode steps
        # remain on the captured graph fast path.
        hyquant_retirement_boundary = (
            not dummy_run
            and self._hyquant_retirement_boundary_needed(batch_req_state)
            # A graph-capturable GPU retirement launch is recorded during
            # capture and checks the live device lengths itself.  Do not
            # switch the complete model step to eager merely because a page
            # boundary is due; retain the eager fallback when graph retirement
            # is explicitly disabled or GPU retirement is unavailable.
            and not (
                self.cache_config.cache_dtype == "hyquant_k4v4"
                and os.environ.get("VLLM_HYQUANT_GPU_RETIRE", "1") != "0"
        ...
        batch_desc, num_tokens_across_dp = dispatch_cg_and_sync_dp(
            ...
            need_eager=is_profile or skip_compiled or hyquant_retirement_boundary,
