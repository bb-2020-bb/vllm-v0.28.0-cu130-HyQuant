...
class InputBuffers:
    def __init__(
        ...
        self.hyquant_state_slots = torch.full(
            (max_num_reqs,), -1, dtype=torch.int32, device=device
        )
        self.hyquant_prompt_lens = torch.zeros(
            max_num_reqs, dtype=torch.int32, device=device
        )
        self.hyquant_cached_prefix_lens = torch.zeros(
            max_num_reqs, dtype=torch.int32, device=device
        )
        self.hyquant_state_slots_cpu = torch.full(
            (max_num_reqs,), -1, dtype=torch.int32
        )
        self.hyquant_prompt_lens_cpu = torch.zeros(max_num_reqs, dtype=torch.int32)
        self.hyquant_cached_prefix_lens_cpu = torch.zeros(
...
class InputBatch:
    ...
    # Stable request-state slot and full-prefill epoch length for HyQuant.
    hyquant_state_slots: torch.Tensor | None
    hyquant_prompt_lens: torch.Tensor | None
    hyquant_cached_prefix_lens: torch.Tensor | None
    ...
    # CPU mirrors avoid synchronizing HyQuant request metadata in every layer.
    hyquant_state_slots_cpu: torch.Tensor | None = None
    hyquant_prompt_lens_cpu: torch.Tensor | None = None
    hyquant_cached_prefix_lens_cpu: torch.Tensor | None = None
    ...
    def make_dummy(
        ...
        return cls(
            ...
            prompt_lens=None,
            hyquant_state_slots=input_buffers.hyquant_state_slots[:num_reqs],
            hyquant_prompt_lens=input_buffers.hyquant_prompt_lens[:num_reqs],
            hyquant_cached_prefix_lens=input_buffers.hyquant_cached_prefix_lens[
                :num_reqs
            ],
            max_query_len=max_query_len,
            hyquant_state_slots_cpu=input_buffers.hyquant_state_slots_cpu[:num_reqs],
            hyquant_prompt_lens_cpu=input_buffers.hyquant_prompt_lens_cpu[:num_reqs],
            hyquant_cached_prefix_lens_cpu=(
                input_buffers.hyquant_cached_prefix_lens_cpu[:num_reqs]
            ),
        )
