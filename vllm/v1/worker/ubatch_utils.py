...
def _make_metadata_with_slice(
    ...
    return CommonAttentionMetadata(
        ...
        hyquant_state_slots=(
            attn_metadata.hyquant_state_slots[request_slice]
            if attn_metadata.hyquant_state_slots is not None
            else None
        ),
        hyquant_state_slots_cpu=(
            attn_metadata.hyquant_state_slots_cpu[request_slice]
            if attn_metadata.hyquant_state_slots_cpu is not None
            else None
        ),
        hyquant_prompt_lens=(
            attn_metadata.hyquant_prompt_lens[request_slice]
            if attn_metadata.hyquant_prompt_lens is not None
            else None
        ),
        hyquant_prompt_lens_cpu=(
            attn_metadata.hyquant_prompt_lens_cpu[request_slice]
            if attn_metadata.hyquant_prompt_lens_cpu is not None
            else None
        ),
        hyquant_cached_prefix_lens=(
            attn_metadata.hyquant_cached_prefix_lens[request_slice]
            if attn_metadata.hyquant_cached_prefix_lens is not None
            else None
        ),
        hyquant_cached_prefix_lens_cpu=(
            attn_metadata.hyquant_cached_prefix_lens_cpu[request_slice]
            if attn_metadata.hyquant_cached_prefix_lens_cpu is not None
            else None
        ),
