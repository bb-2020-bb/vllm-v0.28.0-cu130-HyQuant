...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self.hyquant_state_slots: CpuGpuBuffer | None = None
        self.hyquant_prompt_lens: CpuGpuBuffer | None = None
        self.hyquant_cached_prefix_lens: CpuGpuBuffer | None = None
        self._hyquant_req_to_state_slot: dict[str, int] = {}
        self._hyquant_req_to_prefill_len: dict[str, int] = {}
        self._hyquant_req_to_cached_prefix_len: dict[str, int] = {}
        self._hyquant_free_state_slots: list[int] = []
        if self.cache_config.cache_dtype == "hyquant_k4v4":
            self.hyquant_state_slots = self._make_buffer(
                self.max_num_reqs, dtype=torch.int32
            )
            self.hyquant_state_slots.cpu.fill_(-1)
            self.hyquant_state_slots.gpu.fill_(-1)
            self.hyquant_prompt_lens = self._make_buffer(
                self.max_num_reqs, dtype=torch.int32
            )
            self.hyquant_cached_prefix_lens = self._make_buffer(
                self.max_num_reqs, dtype=torch.int32
            )
            self._hyquant_free_state_slots = list(reversed(range(self.max_num_reqs)))
    ...
    def _on_request_state_removed(
        ...
        del req_state
        state_slot = self._hyquant_req_to_state_slot.pop(req_id, None)
        self._hyquant_req_to_prefill_len.pop(req_id, None)
        self._hyquant_req_to_cached_prefix_len.pop(req_id, None)
        if state_slot is not None:
            self._hyquant_free_state_slots.append(state_slot)

    def _get_hyquant_state(
        self, req_id: str, prefill_len: int, cached_prefix_len: int
    ) -> tuple[int, int, int]:
        state_slot = self._hyquant_req_to_state_slot.get(req_id)
        if state_slot is not None:
            return (
                state_slot,
                self._hyquant_req_to_prefill_len[req_id],
                self._hyquant_req_to_cached_prefix_len[req_id],
            )
        if not self._hyquant_free_state_slots:
            raise RuntimeError(
                "HyQuant exhausted its request-state slots. This indicates "
                "more live KV-cache requests than --max-num-seqs."
            )
        state_slot = self._hyquant_free_state_slots.pop()
        self._hyquant_req_to_state_slot[req_id] = state_slot
        self._hyquant_req_to_prefill_len[req_id] = prefill_len
        self._hyquant_req_to_cached_prefix_len[req_id] = cached_prefix_len
        return state_slot, prefill_len, cached_prefix_len
    ...
    def _build_attention_metadata(
        ...
        if self.hyquant_state_slots is not None:
            assert self.hyquant_prompt_lens is not None
            assert self.hyquant_cached_prefix_lens is not None
            self.hyquant_state_slots.cpu[:num_reqs_padded].fill_(-1)
            self.hyquant_prompt_lens.cpu[:num_reqs_padded].zero_()
            self.hyquant_cached_prefix_lens.cpu[:num_reqs_padded].zero_()
            for req_index, req_id in enumerate(self.input_batch.req_ids):
                state_slot, prefill_len, cached_prefix_len = self._get_hyquant_state(
                    req_id,
                    int(self.input_batch.num_tokens_no_spec[req_index]),
                    int(self.input_batch.num_computed_tokens_cpu_tensor[req_index]),
                )
                self.hyquant_state_slots.cpu[req_index] = state_slot
                self.hyquant_prompt_lens.cpu[req_index] = prefill_len
                self.hyquant_cached_prefix_lens.cpu[req_index] = cached_prefix_len
            self.hyquant_state_slots.copy_to_gpu(num_reqs_padded)
            self.hyquant_prompt_lens.copy_to_gpu(num_reqs_padded)
            self.hyquant_cached_prefix_lens.copy_to_gpu(num_reqs_padded)
            hyquant_state_slots = self.hyquant_state_slots.gpu[:num_reqs_padded]
            hyquant_prompt_lens = self.hyquant_prompt_lens.gpu[:num_reqs_padded]
            hyquant_cached_prefix_lens = self.hyquant_cached_prefix_lens.gpu[
                :num_reqs_padded
            ]
        ...
        cm_base = CommonAttentionMetadata(
            ...
            hyquant_state_slots_cpu=(
                self.hyquant_state_slots.cpu[:num_reqs_padded]
                if self.hyquant_state_slots is not None
                else None
            ),
            hyquant_prompt_lens=hyquant_prompt_lens,
            hyquant_prompt_lens_cpu=(
                self.hyquant_prompt_lens.cpu[:num_reqs_padded]
                if self.hyquant_prompt_lens is not None
                else None
            ),
            hyquant_cached_prefix_lens=hyquant_cached_prefix_lens,
            hyquant_cached_prefix_lens_cpu=(
                self.hyquant_cached_prefix_lens.cpu[:num_reqs_padded]
                if self.hyquant_cached_prefix_lens is not None
                else None
            ),
        )
