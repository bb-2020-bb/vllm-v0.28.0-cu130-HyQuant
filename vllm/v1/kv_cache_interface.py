...
class KVQuantMode(IntEnum):
    ...
    @property
    def is_hyquant(self) -> bool:
        """True for the periodic-window HyQuant cache format."""
        return self == KVQuantMode.HYQUANT_K4V4
...
def get_kv_quant_mode(kv_cache_dtype: str) -> KVQuantMode:
    ...
    if kv_cache_dtype == "hyquant_k4v4":
        return KVQuantMode.HYQUANT_K4V4
...
class AttentionSpec(KVCacheSpec):
    ...
    prefix_cache_private_tail_tokens: int = 0
    """Prompt suffix that local prefix caching must leave request-private."""

    def __post_init__(self):
        if self.head_size_v is None:
            object.__setattr__(self, "head_size_v", self.head_size)
        if self.prefix_cache_private_tail_tokens < 0:
            raise ValueError("Prefix-cache private tail cannot be negative")
...
class FullAttentionSpec(AttentionSpec):
    ...
    def merge(cls, specs: list[Self]) -> Self:
        ...
        merged_spec = cls(
            ...
            prefix_cache_private_tail_tokens=(
                specs[0].prefix_cache_private_tail_tokens
            ),
...
class RSWASpec(FullAttentionSpec):
    ...
    def merge(cls, specs: list[RSWASpec]) -> RSWASpec:
        ...
        return cls(
            ...
            prefix_cache_private_tail_tokens=(base.prefix_cache_private_tail_tokens),
