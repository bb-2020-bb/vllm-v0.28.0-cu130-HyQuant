...
class KVCacheManager:
    def __init__(
        ...
        private_tail_specs = [
            ...
            if getattr(group.kv_cache_spec, "prefix_cache_private_tail_tokens", 0) > 0
        ]
        self.prefix_cache_private_tail_tokens = max(
            (
                spec.prefix_cache_private_tail_tokens  # type: ignore[attr-defined]
                for spec in private_tail_specs
            ),
            default=0,
        )
        self.prefix_cache_private_tail_block_size = (
            lcm(*(spec.block_size for spec in private_tail_specs))
            if private_tail_specs
    ...
    def _get_cacheable_prefix_length(self, request: Request) -> int:
        """Return the prompt prefix whose physical pages are shareable."""
        if self.prefix_cache_private_tail_tokens == 0:
            return request.num_prompt_tokens
        candidate = max(
            request.num_prompt_tokens - self.prefix_cache_private_tail_tokens,
            0,
        )
        block_size = self.prefix_cache_private_tail_block_size
        return candidate // block_size * block_size
    ...
    def get_computed_blocks(self, request: Request) -> tuple[KVCacheBlocks, int, int]:
        ...
        if self.prefix_cache_private_tail_tokens:
            max_cache_hit_length = min(
                max_cache_hit_length,
                self._get_cacheable_prefix_length(request),
            )
