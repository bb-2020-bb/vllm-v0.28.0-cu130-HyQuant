...
class CommonAttentionMetadata:
    ...
    hyquant_state_slots: torch.Tensor | None = None
    """Stable request-state slots used by the HyQuant BF16 hot window."""

    hyquant_state_slots_cpu: torch.Tensor | None = None
    """CPU mirror of HyQuant request-state slots for prefill staging."""

    hyquant_prompt_lens: torch.Tensor | None = None
    """Current full-prefill epoch lengths for periodic HyQuant retirement."""

    hyquant_prompt_lens_cpu: torch.Tensor | None = None
    """CPU prompt lengths used for inexpensive eager retirement decisions."""

    hyquant_cached_prefix_lens: torch.Tensor | None = None
    """Immutable compact prefix present when HyQuant first sees each request."""

    hyquant_cached_prefix_lens_cpu: torch.Tensor | None = None
    """CPU mirror used to reject unsupported HyQuant prefix-cache hits."""
    ...
    def unpadded(
        ...
        return CommonAttentionMetadata(
            ...
            hyquant_state_slots=maybe_slice_reqs(self.hyquant_state_slots),
            hyquant_state_slots_cpu=maybe_slice_reqs(self.hyquant_state_slots_cpu),
            hyquant_prompt_lens=maybe_slice_reqs(self.hyquant_prompt_lens),
            hyquant_prompt_lens_cpu=maybe_slice_reqs(self.hyquant_prompt_lens_cpu),
            hyquant_cached_prefix_lens=maybe_slice_reqs(
                self.hyquant_cached_prefix_lens
            ),
            hyquant_cached_prefix_lens_cpu=maybe_slice_reqs(
                self.hyquant_cached_prefix_lens_cpu
            ),
        )
