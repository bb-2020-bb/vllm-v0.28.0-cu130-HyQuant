...
def make_local_attention_virtual_batches(
    ...
    return CommonAttentionMetadata(
        ...
        hyquant_prompt_lens=common_attn_metadata.hyquant_prompt_lens,
        hyquant_prompt_lens_cpu=common_attn_metadata.hyquant_prompt_lens_cpu,
        hyquant_state_slots=common_attn_metadata.hyquant_state_slots,
        hyquant_state_slots_cpu=common_attn_metadata.hyquant_state_slots_cpu,
        hyquant_cached_prefix_lens=common_attn_metadata.hyquant_cached_prefix_lens,
        hyquant_cached_prefix_lens_cpu=(
            common_attn_metadata.hyquant_cached_prefix_lens_cpu
        ),
    ), make_block_table
...
def split_decodes_and_prefills(
    ...
    if not treat_short_extends_as_decodes:
        ...
        # Full CUDA graphs pad query_start_loc/num_reqs to the captured batch
        # size, while is_prefilling describes only the requests being replayed.
        # The trailing zero-length requests are graph padding, not prefills.
        request_is_prefilling = common_attn_metadata.is_prefilling
        num_request_rows = request_is_prefilling.shape[0]
        assert num_request_rows <= is_prefill.shape[0]
        is_prefill[:num_request_rows] |= request_is_prefilling
