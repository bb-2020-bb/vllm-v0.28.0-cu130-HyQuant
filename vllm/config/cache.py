...
class CacheConfig:
    ...
    """
    hyquant_top_ratio: float = Field(default=0.0625, ge=0.0, le=1.0)
    """Fraction of tokens in each retired block retained as BF16 anchors.

    This option is used only with ``cache_dtype='hyquant_k4v4'``.  The count
    is rounded up to a whole token and the fixed mixed-token page is sized for
    that count, so the allocator reports the actual compact KV footprint.
    """
    hyquant_group_size: int = Field(default=32, ge=2)
    """Channel group size for K/V INT4 scales inside a quantized block.

    K and V have separate scales for every token and channel group.  The
    value must be even and is normally 32 or 64 for head size 128.
    """
    hyquant_window_size: int = Field(default=256, ge=16)
    """Number of recent tokens retained in BF16 by HyQuant."""
    hyquant_retire_interval: int = Field(default=64, ge=16)
    """Decode tokens between HyQuant retirement/quantization batches."""
    ...
    def _validate_cache_dtype(cls, cache_dtype: CacheDType) -> CacheDType:
        ...
        elif cache_dtype == "hyquant_k4v4":
            logger.info(
                "Using %s data type with compact mixed-token groupwise K4/V4 "
                "pages; BF16 anchors and INT4 payloads are decoded online.",
                str(cache_dtype),
            )
