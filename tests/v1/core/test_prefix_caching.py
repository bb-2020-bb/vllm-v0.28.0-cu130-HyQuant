...
def make_private_tail_kv_cache_config(
    block_size: int, num_blocks: int, private_tail_tokens: int
) -> KVCacheConfig:
    return KVCacheConfig(
        num_blocks=num_blocks,
        kv_cache_tensors=[],
        kv_cache_groups=[
            KVCacheGroupSpec(
                ...
                FullAttentionSpec(
                    ...
                    head_size=1,
                    dtype=torch.float32,
                    prefix_cache_private_tail_tokens=private_tail_tokens,
                ),
            )
        ],
    )
...
@pytest.mark.parametrize("prompt_length", [16, 32, 47, 80, 87])
def test_prefix_cache_private_tail_only_shares_complete_prompt_blocks(
    prompt_length: int,
):
    block_size = 16
    private_tail = 32
    manager = make_kv_cache_manager(
        make_private_tail_kv_cache_config(block_size, 16, private_tail),
        max_model_len=256,
        enable_caching=True,
        hash_block_size=block_size,
    )
    ...
    computed, num_computed, _ = manager.get_computed_blocks(first)
    assert num_computed == 0
    blocks = manager.allocate_slots(first, prompt_length, 0, computed)
    assert blocks is not None

    cacheable = max(prompt_length - private_tail, 0) // block_size * block_size
    cacheable_blocks = cacheable // block_size
    first_blocks = manager.get_blocks(first.request_id).blocks[0]
    assert all(
        block.block_hash is not None for block in first_blocks[:cacheable_blocks]
    )
    assert all(block.block_hash is None for block in first_blocks[cacheable_blocks:])

    manager.free(first)
    replay = make_request("replay", prompt, block_size, sha256)
    computed, num_computed, _ = manager.get_computed_blocks(replay)
    assert num_computed == cacheable
    assert len(computed.blocks[0]) == cacheable_blocks


def test_prefix_cache_private_tail_never_registers_generated_blocks():
    block_size = 16
    manager = make_kv_cache_manager(
        make_private_tail_kv_cache_config(block_size, 16, 32),
        max_model_len=256,
        enable_caching=True,
        hash_block_size=block_size,
    )
    prompt = list(range(80))
    request = make_request("first", prompt, block_size, sha256)
    blocks = manager.allocate_slots(request, len(prompt))
    assert blocks is not None

    request.num_computed_tokens = len(prompt)
    generated = list(range(1000, 1032))
    request.append_output_token_ids(generated)
    new_blocks = manager.allocate_slots(request, len(generated))
    assert new_blocks is not None
    request.num_computed_tokens += len(generated)
    manager.cache_blocks(request, request.num_computed_tokens)

    request_blocks = manager.get_blocks(request.request_id).blocks[0]
    assert all(block.block_hash is not None for block in request_blocks[:3])
    assert all(block.block_hash is None for block in request_blocks[3:])
    manager.free(request)

    replay = make_request("replay", prompt + generated, block_size, sha256)
    computed, num_computed, _ = manager.get_computed_blocks(replay)
    assert num_computed == 48
    assert len(computed.blocks[0]) == 3


def test_prefix_cache_private_tail_eviction_releases_all_references():
    block_size = 16
    manager = make_kv_cache_manager(
        make_private_tail_kv_cache_config(block_size, 9, 32),
        max_model_len=256,
        enable_caching=True,
        hash_block_size=block_size,
    )
    first_prompt = list(range(80))
    first = make_request("first", first_prompt, block_size, sha256)
    computed, num_computed, _ = manager.get_computed_blocks(first)
    blocks = manager.allocate_slots(first, len(first_prompt), num_computed, computed)
    assert blocks is not None
    manager.free(first)
    assert manager.block_pool.free_block_queue.num_free_blocks == 8

    second_prompt = list(range(1000, 1128))
    second = make_request("second", second_prompt, block_size, sha256)
    computed, num_computed, _ = manager.get_computed_blocks(second)
    blocks = manager.allocate_slots(second, len(second_prompt), num_computed, computed)
    assert blocks is not None and len(blocks.blocks[0]) == 8
    manager.free(second)

    first_replay = make_request("first-replay", first_prompt, block_size, sha256)
    computed, num_computed, _ = manager.get_computed_blocks(first_replay)
    assert num_computed == 0
    assert not computed.blocks[0]

    second_replay = make_request("second-replay", second_prompt, block_size, sha256)
    computed, num_computed, _ = manager.get_computed_blocks(second_replay)
    assert num_computed == 96
    assert len(computed.blocks[0]) == 6
    blocks = manager.allocate_slots(
        second_replay,
        len(second_prompt) - num_computed,
        num_computed,
        computed,
    )
    assert blocks is not None
    request_blocks = manager.get_blocks(second_replay.request_id).blocks[0]
    assert len(request_blocks) == 8
    assert all(block.ref_cnt == 1 for block in request_blocks)

    manager.free(second_replay)
    assert manager.block_pool.free_block_queue.num_free_blocks == 8
    assert all(block.ref_cnt == 0 for block in manager.block_pool.blocks[1:])
