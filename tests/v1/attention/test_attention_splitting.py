...
def test_make_metadata_with_slice_preserves_hyquant_fields(mixed_small_metadata):
    metadata = mixed_small_metadata
    metadata.positions = torch.arange(metadata.num_actual_tokens)
    metadata.is_prefilling = torch.tensor([False, True, True])
    metadata.hyquant_state_slots = torch.tensor([7, 11, 13])
    metadata.hyquant_state_slots_cpu = torch.tensor([7, 11, 13])
    metadata.hyquant_prompt_lens = torch.tensor([32, 40, 48])
    metadata.hyquant_prompt_lens_cpu = torch.tensor([32, 40, 48])
    metadata.hyquant_cached_prefix_lens = torch.tensor([0, 16, 32])
    metadata.hyquant_cached_prefix_lens_cpu = torch.tensor([0, 16, 32])

    result = _make_metadata_with_slice(UBatchSlice(slice(1, 3), slice(1, 7)), metadata)

    assert torch.equal(result.positions, torch.arange(1, 7))
    assert torch.equal(result.is_prefilling, torch.tensor([True, True]))
    assert torch.equal(result.hyquant_state_slots, torch.tensor([11, 13]))
    assert torch.equal(result.hyquant_state_slots_cpu, torch.tensor([11, 13]))
    assert torch.equal(result.hyquant_prompt_lens, torch.tensor([40, 48]))
    assert torch.equal(result.hyquant_prompt_lens_cpu, torch.tensor([40, 48]))
    assert torch.equal(result.hyquant_cached_prefix_lens, torch.tensor([16, 32]))
    assert torch.equal(
        result.hyquant_cached_prefix_lens_cpu, torch.tensor([16, 32])
    )


def test_unpadded_preserves_hyquant_fields(mixed_small_metadata):
    metadata = mixed_small_metadata
    metadata.hyquant_state_slots = torch.tensor([7, 11, -1])
    metadata.hyquant_state_slots_cpu = torch.tensor([7, 11, -1])
    metadata.hyquant_prompt_lens = torch.tensor([32, 40, 0])
    metadata.hyquant_prompt_lens_cpu = torch.tensor([32, 40, 0])
    metadata.hyquant_cached_prefix_lens = torch.tensor([0, 0, 0])
    metadata.hyquant_cached_prefix_lens_cpu = torch.tensor([0, 0, 0])

    result = metadata.unpadded(num_actual_tokens=2, num_actual_reqs=2)

    assert torch.equal(result.hyquant_state_slots, torch.tensor([7, 11]))
    assert torch.equal(result.hyquant_state_slots_cpu, torch.tensor([7, 11]))
    assert torch.equal(result.hyquant_prompt_lens, torch.tensor([32, 40]))
    assert torch.equal(result.hyquant_prompt_lens_cpu, torch.tensor([32, 40]))
    assert torch.equal(result.hyquant_cached_prefix_lens, torch.tensor([0, 0]))
    assert torch.equal(
        result.hyquant_cached_prefix_lens_cpu, torch.tensor([0, 0])
    )
...
def test_split_decodes_and_prefills_padded_batch_with_unpadded_prefill_flags():
    """Full-CG request padding is not represented in is_prefilling."""
    result = apply_split_decodes_and_prefills(
        [1, 1, 1, 0],
        decode_threshold=1,
